...
class GroqScoutClient:
    ...
    def interpret_emojis(self, user_message: str, language: str = "en", context_history: List[Dict[str,str]] = None) -> Optional[str]:
        system_prompt = (
            "ROLE: You are a compassionate mental health support assistant. The user shared emojis or emoji-heavy text. "
            "GOAL: Offer brief, supportive help – not translation or deep analysis. "
            "RESPONSE RULES: "
            "1) Begin with one short sentence of reassurance (e.g., it’s understandable to feel this way; you’re not alone). "
            "2) Suggest 1–2 simple ways they could respond or set a boundary (short, practical). "
            "3) Ask ONE gentle check-in question. "
            "STYLE: No headings, no section labels, no bullet lists longer than 2 items, no markdown. Keep it under ~120 words. "
        ...
        payload = {"model": self.model, "messages": messages, "max_tokens": 220, "temperature": 0.4}
    ...
    def interpret_image(self, user_message: str, image_b64: str, language: str = "en", context_history: List[Dict[str,str]] = None) -> Optional[str]:
        system_prompt = (
            "ROLE: You are a compassionate mental health support assistant. The user shared an image. "
            "GOAL: Offer brief, supportive help – not a detailed description or analysis. "
            "RESPONSE RULES: "
            "1) Start with one short, calming reassurance (e.g., this looks tough; you don’t have to handle it alone). "
            "2) Offer 1–2 simple next steps they can take (e.g., how to reply, set a boundary, or take care of themselves). "
            "3) Ask ONE gentle follow-up question. "
            "STYLE: No headings, no section labels, no markdown, no long lists; keep it under ~120 words. "
            "LANGUAGE: Write entirely in language code: " + language
        )
        messages = [{"role": "system", "content": system_prompt}]
        if context_history:
        ...
        payload = {"model": self.model, "messages": [*messages, {"role": "user", "content": user_content}], "max_tokens": 260, "temperature": 0.35}
