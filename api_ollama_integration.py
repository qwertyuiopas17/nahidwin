...
- Keep responses focused and relevant."""
    ...
    def generate_mental_health_response(self, user_message: str, user_intent: str, conversation_stage: str, 
                                      ...
                                      emotional_state: str = "neutral", urgency_level: str = "low", 
                                      custom_prompt: str = None) -> Optional[str]:
        """Generates a response using API with context-aware prompt"""
...
SAHARA: [Your response here]"""
        ...
        try:
            self.logger.info("Generating response with context-aware prompt...")
            response = self.client.generate_response(final_prompt)
            
            if response:
                # Clean up the response to remove any template artifacts
                return response.replace("SAHARA:", "").strip()
            return None
    ...
    def build_system_prompt(self, intent: str, stage: str, severity: float, emotional_state: str, urgency_level: str) -> str:
