...
class GroqScoutClient:
    ...
    def __init__(self, api_key: str = None, base_url: str = "https://api.groq.com/openai/v1", model: str = "meta-llama/llama-guard-4-12b"):
    ...
    def interpret_emojis(self, user_message: str, language: str = "en", context_history: List[Dict[str,str]] = None) -> Optional[str]:
        system_prompt = (
            "You are a compassionate mental health support assistant. The user provided emojis or emoji-heavy text. "
            "Your job: 1) briefly interpret what the emojis could mean in everyday communication (do not over-pathologize), "
            "2) provide supportive guidance on how they might respond or set boundaries, "
            "3) keep the tone empathetic and safe. Respond fully in the requested language code: " + language
        )
        ...
        payload = {"model": self.model, "messages": messages, "max_tokens": 500, "temperature": 0.5}
    ...
    def interpret_image(self, user_message: str, image_b64: str, language: str = "en", context_history: List[Dict[str,str]] = None) -> Optional[str]:
        system_prompt = (
            "You are a compassionate mental health support assistant. The user shared an image. "
            "Your job: 1) neutrally describe what the image likely conveys without unsafe inferences, "
            "2) translate what it could mean in social/relationship context, and 3) give supportive next-step suggestions. "
            "Respond fully in the requested language code: " + language
        )
        messages = [{"role": "system", "content": system_prompt}]
        if context_history:
            for msg in context_history[-6:]:
        ...
        user_content = [
            {"type": "text", "text": user_message or "Please interpret this image and advise."},
            {"type": "image_url", "image_url": {"url": f"data:image/jpeg;base64,{image_b64}"}}
        ]
        payload = {"model": self.model, "messages": [*messages, {"role": "user", "content": user_content}], "max_tokens": 600, "temperature": 0.4}
