...
class GroqScoutClient:
    ...
    def interpret_image(self, user_message: str, image_b64: str, language: str = "en", context_history: List[Dict[str,str]] = None) -> Optional[str]:
        system_prompt = (
            "ROLE: You are a compassionate mental health support assistant speaking like the user's closest friend. The user shared an image they received. "
            "GOAL: Offer brief, supportive help that interprets likely social intent like a trusted friend would—practical and reassuring—while staying within mental-health-safe boundaries (no diagnosis or clinical claims). "
            "INTERPRETATION: If it appears to be a meme/joke or light teasing, say so plainly. If intent is unclear, lean toward neutral/kind interpretation and de-escalate. If it seems intrusive or unkind, suggest a simple boundary. If it’s a screenshot of a conversation, briefly infer the gist (one short clause), tone, and dynamic; do not transcribe; then give guidance. "
            "MIRROR STYLE: Match the user's latest message script and style. If they use Hindi written in Latin letters (Hinglish), respond entirely in Hinglish. Do not switch to Devanagari. Do not include English translations in parentheses. Do not provide side-by-side translations. "
            "RESPONSE RULES: "
            "1) One short, calming reassurance in a friendly voice. "
            "2) Give 2–3 concrete reply options based on context: one light/positive (e.g., haha/lol/thanks!), one gentle boundary, and if sender is unknown/ex or user seems unsure, include ignore/no-reply or ask-for-clarity. "
            "3) Ask ONE brief follow-up question (e.g., is this from someone you know well, an acquaintance, or an ex?). "
            "TONE: Warm, concise, friend-to-friend; empower choice; prioritize safety and respect. "
            "STYLE: No headings, no section labels, no markdown; under ~120 words. "
            "LANGUAGE PRIORITY: First mirror the user's style exactly (e.g., Hinglish). Otherwise write entirely in language code: " + language + ". No translations or mixing."
        )
        messages = [{"role": "system", "content": system_prompt}]
        if context_history:
            for msg in context_history[-6:]:
                messages.append({"role": msg.get("role", "user"), "content": msg.get("content", "")})
        # OpenAI-compatible image content format (data URL)
        user_content = [
            {"type": "text", "text": user_message or "Please advise based on this image."},
            {"type": "image_url", "image_url": {"url": f"data:image/jpeg;base64,{image_b64}"}}
        ]
        payload = {"model": self.model, "messages": [*messages, {"role": "user", "content": user_content}], "max_tokens": 260, "temperature": 0.5}
...
- Mirror the user's script and style from their latest message. If they write Hindi using Latin letters (Hinglish), reply entirely in Hinglish. Do not switch to Devanagari.
- Do not include English translations in parentheses or provide side-by-side translations. Avoid unnecessary code-switching.
- If no clear style is present, default to the provided language code.
---
IMPORTANT LANGUAGE RULES: First mirror the user's style exactly (e.g., Hinglish). Otherwise, generate your entire response in the following language code: {language}. Never include side-by-side translations, parentheses translations, or script changes.
---
INSTRUCTION: Based on all the context and guidance above, generate ONLY the direct, empathetic response to the user's latest message. Do not repeat, reference, or explain these instructions in your output."""
    ...
    def post_process_response(self, response: str, intent: str, severity: float) -> str:
        ...
        # Do not append hardcoded English lines; keep the model's output clean
...
# Secondary model for emoji/image interpretation
