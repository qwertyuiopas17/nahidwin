...
from api_ollama_integration import ollama_llama3, groq_scout
...
def predict():
    ...
    try:
        ...
        # language_code is now detected automatically below, removing the need to trust the request
        ...
        # --- FIX START: Dynamic Language Detection ---
        # The language is now determined by the user's message, not a frontend setting.
        detected_language_code = "en" # Default to English
        if user_message:
            try:
                # Detect the language from the user's message content.
                lang = detect(user_message)
                # Normalize languages. 'hi' for Hindi. For Hinglish (Latin script),
                # it will often detect 'en', which is correct behavior as we instruct the AI
                # to mirror the user's style in the prompt.
                if lang == 'hi':
                    detected_language_code = 'hi'
                else:
                    detected_language_code = 'en'
                logger.info(f"Language detected as '{detected_language_code}' for user '{user_id_str}'.")
            except LangDetectException:
                # If detection fails (e.g., for very short messages like "hi" or "ok"), default to English.
                logger.warning(f"Could not detect language for message from user '{user_id_str}'. Defaulting to 'en'.")
                detected_language_code = "en"
        # --- FIX END ---
        ...
        if multimodal_triggered and groq_scout and groq_scout.is_available:
            logger.info("🛰️ Using Groq Llama 4 Scout for emoji/image interpretation")
            if image_b64:
        ...
        crisis_analysis = crisis_detector.detect_crisis_with_context(
            effective_message, conversation_history=recent_conversation
        )
        ...
        if multimodal_triggered and groq_scout and groq_scout.is_available:
            response_data = {
                'response': clean_ai_response(scout_text),
                'conversation_stage': 'understanding',
                'intent_addressed': nlu_understanding['primary_intent'],
                'severity_addressed': nlu_understanding['severity_score'],
                'emotional_state_addressed': nlu_understanding.get('emotional_state', {}).get('primary_emotion', 'neutral'),
            ...
            response_data = response_generator.generate_response(
                ...
                user_id=user_id_str,
                language=detected_language_code, # <-- MODIFIED LINE: Use the detected language
        ...
        if intent not in concerns and intent not in ['general_support', 'help_seeking']:
            ...
            current_user.set_primary_concerns(concerns[-5:])
        ...
        logger.debug("Step 10: Prepare Comprehensive Enhanced Response...")
        with app.app_context():
            current_user = User.query.filter_by(student_id=user_id_str, is_active=True).first()
