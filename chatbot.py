...
def clean_ai_response(text: str) -> str:
    """
    Forcefully removes known instructional artifacts and prompt leakage from the AI's final response.
    This acts as a safety net.
    """
    if not isinstance(text, str):
        return text

    # Remove any XML-style tags used in the prompt
    text = re.sub(r'<[/]?system_prompt>|<[/]?task_instructions>|<[/]?conversation_history>|<[/]?response_template>', '', text, flags=re.IGNORECASE)

    # Remove common instruction phrases that might leak
    phrases_to_remove = [
        "Your task is to",
    ...
    for phrase in phrases_to_remove:
        text = re.sub(r'\b' + re.escape(phrase) + r'.*$', '', text, flags=re.MULTILINE | re.IGNORECASE)

    # Remove conversational role markers
    text = text.replace("SAHARA:", "").replace("User:", "")
...
app.config.update({
    'SQLALCHEMY_DATABASE_URI': f'sqlite:///{os.path.join(instance_path, "enhanced_chatbot.db")}',
...
def predict():
    ...
    try:
       ...
       # --- FIX START: Gather and pass conversation history to NLU ---
        ...
        if (should_refer_counselor and not method_suggestion and 
            response_data['conversation_stage'] in appropriate_stages_for_referral):
            
            logger.info(f"🏥 Generating counselor referral for {user_id_str}")
            ...
            # Create counselor referral response
            referral_text = (
                f"\n\n**Professional Support Recommendation**\n\n"
                f"Based on our conversations, I think working with **{counselor_info['name']}** "
                f"could provide you with additional support and specialized techniques.\n\n"
                ...
                f"Would you like me to provide more information about how to get started with professional counseling?"
            )
