...
def predict():
    ...
    try:
        ...
        # Detect emojis quickly (basic heuristic)
       ...
       # --- FIX START: Gather and pass conversation history to NLU ---
        ...
        # If input is image or emoji-heavy, use Groq Scout model to interpret first
        if multimodal_triggered and groq_scout and groq_scout.is_available:
            ...
            if image_b64:
                scout_text = groq_scout.interpret_image(user_message=user_message, image_b64=image_b64, language=language_code, context_history=nlu_history)
            else:
                scout_text = groq_scout.interpret_emojis(user_message=user_message, language=language_code, context_history=nlu_history)
            # Fallback if scout not returned
            if not scout_text:
                scout_text = user_message
            # Feed the interpreted text into NLU as the working message
        ...
        if multimodal_triggered and groq_scout and groq_scout.is_available:
            # Use Scout output directly as the assistant response in this turn
            response_data = {
